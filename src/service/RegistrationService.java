...
public class RegistrationService implements Registrable {
    ...
    // Implement the interface's register method
    @Override
    public void register(User user, Course course) throws CourseFullException {
        registerStudentToCourse(user.getId(), course.getId());
    }

    // Implement the interface's drop method
    @Override
    public void drop(User user, Course course) {
        dropCourse(user.getId(), course.getId());
    }

    // Original method using IDs
    public void registerStudentToCourse(int studentId, int courseId) throws CourseFullException {
        try (Connection conn = Database.getConnection()) {
            // Check if already registered
            String checkSql = "SELECT * FROM registrations WHERE student_id = ? AND course_id = ?";
